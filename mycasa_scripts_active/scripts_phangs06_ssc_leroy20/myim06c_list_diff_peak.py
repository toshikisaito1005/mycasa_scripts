...
for i in range(len(dir_gals)):
       ...
       diff_total_model = np.max(image_model[image_model>0])
       #
       ...
       diff_total_7m = (diff_total_model - np.max(image_7m[image_7m>0])) / diff_total_model
       #
       image_feather = imval(image_feather,box=box)["data"].flatten()
       image_feather[np.isnan(image_feather)] = 0
       diff_total_feather = (diff_total_model - np.max(image_feather[image_feather>0])) / diff_total_model
       #
       image_tp2vis = imval(image_tp2vis,box=box)["data"].flatten()
       image_tp2vis[np.isnan(image_tp2vis)] = 0
       diff_total_tp2vis = (diff_total_model - np.max(image_tp2vis[image_tp2vis>0])) / diff_total_model
       #
       image_tpmodel = imval(image_tpmodel,box=box)["data"].flatten()
       image_tpmodel[np.isnan(image_tpmodel)] = 0
       diff_total_tpmodel = (diff_total_model - np.max(image_tpmodel[image_tpmodel>0])) / diff_total_model
       #
...
np.savetxt("list_diff_peak.txt",list_median,header="area 7m feather tp2vis tpmodel",fmt="%.3f")
