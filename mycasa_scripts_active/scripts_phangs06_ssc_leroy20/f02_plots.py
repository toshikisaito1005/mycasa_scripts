...
def plotter(
       ...
       #
       plt.scatter(data[:,0]*2, data[:,1], color=cm.gnuplot(0/3.5), s=50, alpha=0.7, lw=1, label=label_7m)
       plt.scatter(data[:,0]*2, data[:,2], color=cm.gnuplot(1/3.5), s=50, alpha=0.7, lw=1, label=label_feather)
       plt.scatter(data[:,0]*2, data[:,3], color=cm.gnuplot(2/3.5), s=50, alpha=0.7, lw=1, label=label_tp2vis)
       plt.scatter(data[:,0]*2, data[:,4], color=cm.gnuplot(3/3.5), s=50, alpha=0.7, lw=1, label=label_tpmodel)
       #
       plt.xlim([0,300])
       plt.ylim(ylim)
       plt.legend(ncol=2)
...
### Main Procedure
#####################
plotter("list_median.txt", dir_data+"fig_fidelity_vs_circ_diameter.png", [0,55])
