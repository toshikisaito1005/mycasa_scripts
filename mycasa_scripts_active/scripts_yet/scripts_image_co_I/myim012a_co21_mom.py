...
image_co21 = glob.glob(dir_data+"data/"+galname+"*image*")[1]
...
nchan = imhead(pbimage)["shape"][2]
pbimage = pbimage + ".regrid"

if nchan > 1:
    imsubimage(imagename = pbimage,
               chans = str(nchan/2),
               overwrite = True,
               outfile = pbimage + "2")
    os.system("rm -rf " + pbimage)
    os.system("mv " + pbimage + "2 " + pbimage)
...
ia.close()
os.chdir("/Users/saito/data/mycasa_scripts/scripts_image_phangs_ulirg/")
