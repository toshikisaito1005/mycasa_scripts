...
for i in range(1000):
	...
	plt.subplots_adjust(bottom=0.05, left=0.05, right=0.98, top=0.95)
	...
	ax1.hist(histdata10[:,0], range=[-0.015,0.015], bins=11, label="co10 norm mean")
	ax2.hist(histdata10[:,1], range=[-0.02, 0.02], bins=11, label="co10 norm disp")
	ax3.hist(np.log10(np.array(histdata10[:,2])), range=[-3.0, 0.0], bins=11, label="log co10 scatter")
	ax4.hist(histdata21[:,0], range=[1.05, 1.15],bins=11, label="co10 vs co21 slope")
	ax5.hist(histdata21[:,1], range=[-0.8, -0.2], bins=11, label="co10 vs co21 intercept")
	ax6.hist(np.log10(np.array(histdata21[:,2])), range=[-1.0, -0.11394335], bins=11, label="log co10 scatter")
	ax1.legend()
	ax2.legend()
	ax3.legend()
	ax4.legend()
