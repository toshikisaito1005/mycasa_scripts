...
range_scatters_co10 = [[np.log10(0.01), np.log10(1.5)],
					   ...
					   [np.log10(0.01), np.log10(1.5)]]
range_slopes_co21 = [[1.00,1.10], # [1.12,1.18], [1.12,1.18], [1.10,1.16], [1.10,1.16], [1.00,1.10]
					 [0.98,1.04], # [1.00,1.06], [1.00,1.06], [0.98,1.04], [0.96,1.02], [0.98,1.04]
					 [0.96,1.02]] # [1.24,1.34], [1.10,1.15], [0.96,1.02], [0.96,1.02], [0.96,1.02]
range_intercepts_co21 = [[-0.60,-0.30], # [-0.60,-0.30], [-0.60,-0.30], [-0.60,-0.30], [-0.60,-0.30], [-0.60,-0.30]
						 [-0.40,0.10], # [-0.40,0.10], [-0.40,0.10], [-0.40,0.10], [-0.40,0.10], [-0.40,0.10]
						 [-0.50,-0.10]] # [-1.40,-0.40], [-1.00,0.00], [-0.50,-0.10], [-0.50,-0.10], [-0.50,-0.10]
range_scatters_co21 = [[np.log10(0.003),np.log10(0.01)], # [(1.30),(1.55)], [(0.70),(0.90)], [(0.50),(0.70)], [(0.03),(0.30)], [(0.03),(0.30)]
					   [np.log10(0.5),np.log10(1.5)], # [(2.5),(3.5)], [(1.5),(2.5)], [(1.2),(2.2)], [(1.0),(2.0)], [(1.0),(2.0)]
					   [np.log10(0.8),np.log10(1.2)]] # [(1.0),(1.8)], [(1.0),(1.5)], [(1.0),(1.4)], [(0.8),(1.2)], [(0.8),(1.2)]
#
nbins = nbins[i]
beams = beams[i]
scale = scales[i]
...
for j in range(len(beams)):
	...
	for i in range(100):
		...
		plt.savefig(dir_proj + "eps/fig_model_param_"+galname+"_"+this_beam+".png",dpi=200)
		#
		range_co10_input = [log_co10_mom0_k.min(), log_co10_mom0_k.max()]
		range_co21_input = [log_co21_mom0_k.min(), log_co21_mom0_k.max()]
