...
freqco10 = 115.27120
freqco21 = 230.53800
nbins = [15] # 10, 10, 20, 30, 40 # 30, 20, 20, 10, 10 # 40, 30, 25, 20, 15
percentile = 84
...
def get_best_co10_parameter(
	...
	range_scatter = np.logspace(np.log10(0.01), np.log10(1.5), 11) # np.logspace(np.log10(0.01), np.log10(1.5), 11) ngc628
	#
	list_popt1 = []
	list_popt2 = []
	list_scatter = []
...
def get_best_co21_parameter(
	...
	#
	range_slope = np.linspace(1.00, 1.15, 16) #1.05-1.40, 1.05-1.40, 1.05-1.30, 1.05-1.30, 1.05-1.30 (n0628), 0.95-1.10 (n3627), 1.00-1.15 (n4321)
	range_intercept = np.linspace(-0.80, -0.00, 11) # -0.45-020 (n0628), -0.40-0.10 (n3627), -1.00--0.50, -1.00--0.50, -0.80--0.00, -0.80--0.00 (n4321)
	range_scatter = np.logspace(np.log10(0.2), np.log10(1.0), 11) # 0.5-1.8, 0.5-1.3, 0.5-1.0, 0.0-0.5, 0.0-0.2 (n0628), 2.5-3.5, 2.5-3.5, 2.0-3.0, 1.5-2.5, 1.5-2.5 (n3627), 0.5-1.8, 0.5-1.8, 0.5-1.3, 0.5-1.3, 0.2-1.0
	#
