...
for i in range(len(gals)):
    ...
    co21 = co21[cut_all]
    # radial binning
    n, _ = np.histogram(co21, bins=nbins)
    sy, _ = np.histogram(co21, bins=nbins, weights=norm_r21)
    sy2, _ = np.histogram(co21, bins=nbins, weights=norm_r21*norm_r21)
    mean = sy / n
...
ax1.set_xscale("log")
ax1.set_ylim([0,2])
ax1.set_xlabel("$I_{CO(2-1)}$ (currently Jy/b, change to K)")
