...
def cube_to_moments(pixelmin,
    ...
    easy_imregrid(pbimage,image_co21+".template",axes=[0,1],delete_original=False)
    ...
    image2smooth21=glob.glob(dir_data+"data/"+galname+"*21*smooth")[0]
    easy_imregrid(image2smooth21,image_co21+".template",axes=[0,1],delete_original=True)
...
def convolve_wise(dir_data,galname,wise_band,beam_native,targetbeam,ra,dec,template):
    """
    """
    print("###########################")
    ...
    easy_imregrid(imagename.replace(".fits",".image")+".beam.smooth",
                  template,
                  axes=[0,1],delete_original=False)
    
    os.system("mv "+imagename.replace(".fits",".image")+".beam.smooth.regrid "+dir_galname+galname+"_"+wise_band+"_"+str(targetbeam).replace(".","p")+".image")
...
def rebinpix(imagename,output,blc_ra,blc_dec):
    ...
    importfits(fitsimage=output+"_tmp_.fits",
               imagename=output)
