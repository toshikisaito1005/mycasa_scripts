...
do_steps = [0,1]
galnames = ["ngc4254"]
beam_sizes = [8.0] # do_steps = 0, 1, 2, 3
apertures = [[36.0]] # do_steps = 0, 1, 2
...
def do_step_0(dir_data, param, beam_size):
    ...
    os.system("rm -rf " + dir_data + param[0] + "/*co10*.mask")
    os.system("rm -rf " + dir_data + param[0] + "/*co21*.mask")
    os.system("rm -rf " + dir_data + param[0] + "/*r21*.mask")
    os.system("rm -rf " + dir_data + param[0] + "/*pb*.mask")
    os.system("rm -rf " + dir_data + param[0] + "/*.mask.all")
    os.system("rm -rf " + dir_data + param[0] + "/*.masked")

    template=glob.glob(dir_data+param[0]+"/*"+str(beam_size).replace(".","p")+"*.moment0")[0]
    r21.convolve_wise(dir_data,param[0],"w1",7.5,beam_size,param[18],param[19],template)
    r21.convolve_wise(dir_data,param[0],"w2",7.5,beam_size,param[18],param[19],template)
    r21.convolve_wise(dir_data,param[0],"w3",7.5,beam_size,param[18],param[19],template)
