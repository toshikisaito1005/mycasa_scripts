...
product_tkin = dir_data + "ngc3110_radex_tkin.txt"
os.system("rm -rf " + product_tkin)
f = open(product_tkin, "a")
...
for i in range(len(r21)):
    ...
    for j in range(len(radex_grids)):
        ...
        best_tkin = Tkin[index]
        if r13[i] == 0.:
            best_tkin = 0.
        if r21[i] == 0.:
            best_tkin = 0.
        tkins.append(best_tkin)

    # export
    data1 = str(tkins[0])+" "+str(tkins[1])+" "+str(tkins[2])+" "
    data2 = data1 + str(tkins[3])+" "+str(tkins[4])
    f = open(product_tkin, "a")
