...
dir_data2 = "../../../ngc3110/ana/datacube_aperture_uvlim/"
...
for i in range(25):
    ...
    for j in range(25):
        ...
        #co10: R
        value = imval(imagename = dir_data2 + "line_12co10_contsub_clean20_nat_uv_smooth.image.regrid.immath.moment0",
                      region = region_file)
        value_masked_co10 = value["data"] * value["mask"]
        beta = 1.222 * 10 ** 6. / 1.813 / 1.811 / 115.27120 ** 2
        ...
        if data_co102 < 0.0011 * 20 * sqrt(27) * beta * 3.:
            data_co102 = 0.0
        #co21: R
        value = imval(imagename = dir_data2 + "line_12co21_contsub_clean20_nat_smooth.image.regrid.immath.moment0",
                      region = region_file)
        value_masked_co21 = value["data"] * value["mask"]
        beta = 1.222 * 10 ** 6. / 1.813 / 1.813 / 230.53800 ** 2
        data_co21 = value_masked_co21.sum(axis = (0, 1)) * beta / 113.
        if data_co21 < 0.0013 * 20 * sqrt(30) * beta * 3.:
            data_co21 = 0.0
        #co21: H
        value = imval(imagename = dir_data2 + "line_12co21_contsub_clean20_nat_smooth.image.regrid.immath.moment0",
                      region = region_file2)
        ...
        if data_co212 < 0.0013 * 20 * sqrt(30) * beta * 3.:
            data_co212 = 0.0
        #13co10: R
        value = imval(imagename = dir_data2 + "line_13co10_contsub_clean20_nat_uv_smooth.image.regrid.immath.moment0",
                      region = region_file)
        value_masked_13co10 = value["data"] * value["mask"]
        beta = 1.222 * 10 ** 6. / 1.813 / 1.813 / 110.20135430 ** 2
        data_13co10 = value_masked_13co10.sum(axis = (0, 1)) * beta / 113.
        if data_13co10 < 0.0012 * 20 * sqrt(22) * beta * 2.5:
            data_13co10 = 0.0
        #13co10: H
        value = imval(imagename = dir_data2 + "line_13co10_contsub_clean20_nat_uv_smooth.image.regrid.immath.moment0",
                      region = region_file2)
        ...
        if data_13co102 < 0.0012 * 20 * sqrt(22) * beta * 2.5:
            data_13co102 = 0.0
        #13co21: R
        value = imval(imagename = dir_data2 + "line_13co21_contsub_clean20_nat_smooth.image.regrid.immath.moment0",
                      region = region_file)
        value_masked_13co21 = value["data"] * value["mask"]
        beta = 1.222 * 10 ** 6. / 1.813 / 1.813 / 220.39868420 ** 2
        data_13co21 = value_masked_13co21.sum(axis = (0, 1)) * beta / 113.
        if data_13co21 < 0.0009 * 20 * sqrt(25) * beta * 3.:
            data_13co21 = 0.0
        #13co21: H
        value = imval(imagename = dir_data2 + "line_13co21_contsub_clean20_nat_smooth.image.regrid.immath.moment0",
                      region = region_file2)
        ...
        if data_13co212 < 0.0009 * 20 * sqrt(25) * beta * 3.:
            data_13co212 = 0.0
        # writing
        f = open(product_file, "a")
