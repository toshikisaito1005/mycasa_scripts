...
for i in [0,1,2,3,4,5,6]:
    ...
    image_co21 = dir_data+gals[i]+"/"+gals[i]+"_co21_"+beam+".moment0"
    ...
    chnoise = ch_noise(image_co21.replace("moment0","cube.depb"),chanss[i])
    rms_ch = imstat(image_co21.replace("moment0","cube"),chans=chnoise)["rms"][0]
    num_ch = int(chanss[i].split("~")[1])-int(chanss[i].split("~")[0])+1
    
    header = imhead(image_co21.replace("moment0","cube.depb"),mode="list")
    velres = round(300000 * abs(header["cdelt4"]) / header["restfreq"][0],0)
    rms = rms_ch * velres * np.sqrt(num_ch)
    flux_Jybkms = value_co21_1d[value_co21_1d > rms * snr_mom]
    barea_pix, pixelsize = beam_area(image_co21.replace("moment0","cube"))
    #flux_Jykms = flux_Jybkms / barea_pix
    lum = 3.25e7 * flux_Jybkms / 230.538**2 * DLs[i]**2 / (1+zs[i])
    mass = lum * aco
    mass_den = mass / (pixelsize**2 * scales[i]**2)
    ...
    value = imval(image_co21,box="0,0,"+str(image_r)+","+str(image_t))
    value_co21 = value["data"] * value["mask"]
    value_co21_1e = value_co21.flatten()

    disp = value_co21_1e[value_co21_1d > rms * snr_mom]

    # plot
    y = np.log10(mass_den * disp**2)
    y_median = np.median(y)
    y_16 = np.percentile(y, 16)
    y_84 = np.percentile(y, 84)
    
    data_x.append(i+1)
    data_y_median.append(y_median)
    data_y_16.append(y_16)
    data_y_84.append(y_84)
