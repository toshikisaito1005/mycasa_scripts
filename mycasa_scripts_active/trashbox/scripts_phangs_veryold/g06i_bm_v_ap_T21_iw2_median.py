...
output = "bm_vs_ap_t21_iw2_median.txt"
...
for i in range(len(txt_files)):
    ...
    f.write(size_ap + " " + size_bm + " " \
            + str(w_median(r_int[r_int>0], 1/r_int[r_int>0])) + " " \
            + str(w_median(r_pea[r_pea>0], 1/r_pea[r_pea>0])) + " " \
            + str(w_avg_and_std(r_int[r_int>0], 1/r_int[r_int>0])[1]) + " " \
            + str(w_avg_and_std(r_pea[r_pea>0], 1/r_pea[r_pea>0])[1]) + " \n")
...
plt.text(1*scale + (35*scale-1*scale)*0.05,
         ...
         "(d) square-inverse-weighted-median $M$ ($\~M_{1/w^2}$)")
cbar=plt.colorbar()
cbar.set_label("$\~M_{1/w^2}$", size=18)
plt.savefig("/Users/saito/data/phangs/co_ratio/eps/f06l_bm_vs_ap_P21_iw2_median_n4321.png",
            dpi=100)
