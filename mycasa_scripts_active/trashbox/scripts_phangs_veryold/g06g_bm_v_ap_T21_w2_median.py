...
output = "bm_vs_ap_t21_w2_median.txt"
...
for i in range(len(txt_files)):
    ...
    f.write(size_ap + " " + size_bm + " " \
            + str(w_median(r_int[r_int>0], r_int[r_int>0]**2)) + " " \
            + str(w_median(r_pea[r_pea>0], r_pea[r_pea>0]**2)) + " " \
            + str(w_avg_and_std(r_int[r_int>0], r_int[r_int>0]**2)[1]) + " " \
            + str(w_avg_and_std(r_pea[r_pea>0], r_pea[r_pea>0]**2)[1]) + " \n")
...
plt.text(1*scale + (35*scale-1*scale)*0.05,
         ...
         "(b) square-weighted-median $M$ ($\~M_{w^2}$)")
cbar=plt.colorbar()
cbar.set_label("$\~M_{w^2}$", size=18)
plt.savefig("/Users/saito/data/phangs/co_ratio/eps/f06j_bm_vs_ap_P21_w2_median_n4321.png",
            dpi=100)
