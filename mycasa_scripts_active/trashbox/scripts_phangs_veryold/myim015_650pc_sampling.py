...
for i in range(len(aperture_650pc)):
    print("##### sampling with aperture = " + str(aperture_650pc[i]))
    # define sampling grid at the given aperture size
    if galname == "ngc3627":
        stp_ra,stp_dec,rng_ra,rng_dec=Nyq.def_step2(aperture_650pc[i],fov)
    else:
        stp_ra,stp_dec,rng_ra,rng_dec=Nyq.def_step(aperture_650pc[i],fov)

    # rms calculation at the given aperture size
    Sa_co,Sb_co = Nyq.def_area(image_co21,aperture_650pc[i],beam_size)
    ...
    if type(aperture_650pc[i]) == int:
        name_size = "{0:02d}".format(aperture_650pc[i])
    elif type(aperture_650pc[i]) == float:
        name_size = str(aperture_650pc[0]).replace(".","p")

    product_file = dir_data+galname+"_flux_650pc_"\
                   +str(name_size)+".txt"
    ...
    Nyq.hexa_sampling(aperture_650pc[i],
                      sampling_images,
                      sampling_thress,
                      ra_dgr,
                      dec_dgr_org,
