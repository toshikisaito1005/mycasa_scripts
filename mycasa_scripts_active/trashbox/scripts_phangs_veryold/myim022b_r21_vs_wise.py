...
def txt2ratio_radial(txtfile,
    ...
    """
    """
    data = np.loadtxt(txtfile, usecols=(0,1,2,3,4,5,6,11))
    ...
    return dval, drad, ddisp, fco21, fw1, fw2, fw3, galmask
...
def plotter(aperture,
    ...
    ax2.set_title("log W1")
    ax4.set_title("log W2")
    ax6.set_title("log W3")
    ...
    ### ax2
    log_fw1 = np.log10(fw1)
    log_xlim = [min(log_fw1)-0.1, max(log_fw1)*1.1]
    ...
    n, _ = np.histogram(log_fw1,bins=6)
    sy, _ = np.histogram(log_fw1,bins=6,weights=dval)
    sy2, _ = np.histogram(log_fw1,bins=6,weights=dval*dval)
    mean = sy / n
    ...
    ### ax3
    edges_x = [0.1*x for x in range(-6,int(max(log_fw1)*2)*10,1)]
    hist_ax3 = ax3.hist(log_fw1,
                        bins=edges_x,
                        alpha=0.4,
                        histtype="stepfilled",
    ...
    ax3.invert_yaxis()
    ax3.set_yticks([])
    ax3.set_xlabel("log W1")

    log_fw1_masked_tmp = np.array(log_fw1) * np.array(galmask)
    log_fw1_masked = []
    for i in range(len(log_fw1_masked_tmp)):
        if galmask[i] > 0.5:
            log_fw1_masked.append(log_fw1_masked_tmp[i])
    
    hist_ax3b = np.histogram(log_fw1_masked,bins=edges_x)
    ...
    ### ax4
    log_fw2 = np.log10(fw2)
    log_xlim = [min(log_fw2)-0.1, max(log_fw2)*1.1]
    ...
    ### ax5
    edges_x = [0.15*x for x in range(-8,int(max(log_fw2)*2)*8,1)]
    ...
    ### ax6
    log_fw3 = np.log10(fw3)
    log_xlim = [min(log_fw3)-0.1, max(log_fw3)*1.2]
    ...
    ### ax7
    edges_x = [0.1*x for x in range(-6,int(max(log_fw3)*2)*10,1)]
...
for i in range(len(txtfiles)):
    ...
    dval, drad, ddisp, fco21, fw1, fw2, fw3, galmask \
        = txt2ratio_radial(txtfiles[i], 4.,ra_dgr, dec_dgr,
                           scale, pa, incl)
    plotter(aperture, drad, dval, ddisp, fco21,
            fw1, fw2, fw3, galmask, histobin_ap,
            savefig=txtfiles[i].replace(".txt","_wise.png"),
            ylim_rval = [0,1.001])
